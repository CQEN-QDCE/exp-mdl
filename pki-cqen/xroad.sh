...
# Créér une CSR pour la demande d'un nouvel certificat.
    ...
    openssl req -new -utf8 -config etc/xroad.cnf -out ./ca/csr/$PARM.csr -keyout ./ca/private/$PARM.key
}
...
# Créér une CSR pour la demande d'un nouvel certificat.
    ...
    openssl req -new -utf8 -config etc/xroad.cnf -out ./ca/csr/$PARM.csr -sha256 -key ./ca/private/$PARM.key 
...
# Genèrer un numéro de série et emet le certificat.
    ...
    openssl ca -create_serial -config etc/xroad.cnf -in ca/csr/$PARM.csr -out certs/users/$PARM.crt -extensions usuario_reqext

    openssl crl2pkcs7 -nocrl -certfile certs/users/$PARM.crt -certfile certs/$ACXROAD.crt -certfile certs/$ACINTER.crt -certfile certs/$ACRACINE.crt -out certs/users/$PARM-chain.p7s -outform der
