...
if [ ! -f "${XROAD_ROOT}/csr/${USERNAME}.csr" ]; then
    echo -e "\033[31mError: ${XROAD_ROOT}/csr/${USERNAME}.csr not found\033[0m"
    exit 1
fi

# Sign the CSR with your CA to create the certificate
openssl ca -batch -config ${XROAD_ROOT}/config/openssl.conf -extensions client_cert -days ${DAYS_VALID} -notext -md sha256 \
    -in ${XROAD_ROOT}/csr/${USERNAME}.csr \
    -out ${XROAD_ROOT}/certs/${USERNAME}.cer \
...
echo -e "\033[32mGenerated certificate: ${XROAD_ROOT}/certs/${USERNAME}.cer\033[0m"
