...
openssl req -new -key ${SAAQ_ROOT}/private/${USERNAME}.key \
            -out ${SAAQ_ROOT}/csr/jws_${USERNAME}.csr \
...
retVal=$?
if [ $retVal -ne 0 ]; then
    echolor $COLOR_RED "[Error] Génération de la CSR du certificat JWT de ${USERNAME}"
    exit 1
fi

echolor $COLOR_GREEN "Generated ECDSA private key (P-256 curve): ${SAAQ_ROOT}/private/${USERNAME}.key"
echolor $COLOR_GREEN "Generated CSR: ${SAAQ_ROOT}/csr/jws_${USERNAME}.csr"
